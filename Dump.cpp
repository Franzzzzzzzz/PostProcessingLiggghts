...
if (!dumpin)
  {DISP_Err("Erreur d'ouverture du dump\n") ; return -1 ;} 
...
if (dumptmp.is_open() && !actions["dumpall"].set )//&& !actions["chainforce"].set)
  ...
  if (Type==0) DISP_Info("Le .tmp est ancien; il ne contient pas de donné de type ...\n") ;
  else if (Type >3) DISP_Warn("La donnée de type est étrange ...") ; 
  ...
  {
    DISP_Info("Le tmp ne sera pas ecrit car trop gros comparé au dump. Utiliser --forcetmp pour contrecarrer cela.\n") ; 
  }
  else
  {
...
if (steps[timestep].filtered==false)
{
  ...
  if (actions["anglebystep"].set)
  {
   ...
   DISP_Err("Please check the type value here (anglebystep in Dump.cpp, check_timestep)\n") ; 
...
if (actions["extract"].set)
   { res[0]=(long int)actions["extract"]["extract_deb"] ; res[2]=(long int)actions["extract"]["extract_fin"] ; 
     ...
     if (res[0]>nbsteps) {DISP_Err("ERR: step initial demandée supérieure au nombre de steps\n\n") ; exit(1) ; }
     if (res[2]<0) {DISP_Err("ERR: step final < 0\n\n") ; exit(1) ;} 
  }
else
